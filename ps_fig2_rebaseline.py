...
if makefig:
    ...
    for i in range(4,-1,-1):
        ...
        pb.errorbar(range(1,21),s[:,i]-s[0,i],yerr=stderr[:,i],marker=markervals[i],color='k',label=labelname)
        #errorbar(range(1,21), s[:,i],yerr=scores_var[:,i],marker=markervals[i],color='k',label=labelname)
           
    plt.legend(loc=2)
    plt.xlabel('Attempt number')
    plt.ylabel('Average score (normalised to baseline of average on attempt 1)')
    plt.xlim([0.5, 20.5])
    plt.ylim([-7000,49000])
    plt.savefig('StaffordFig2rebaseline.png', dpi=300, facecolor='w', edgecolor='w',
            orientation='portrait', papertype=None, format=None,
            transparent=False, bbox_inches=None, pad_inches=0.1)
    
    generatepaperfigs=0
    if generatepaperfigs:    
        plt.savefig('../psycscience/StaffordFig2rebaseline.png', dpi=300, facecolor='w', edgecolor='w',
            orientation='portrait', papertype=None, format=None,
            transparent=False, bbox_inches=None, pad_inches=0.1)
